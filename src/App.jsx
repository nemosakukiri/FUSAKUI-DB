          ...
          <div className="animate-in fade-in duration-700 w-full max-w-5xl space-y-24">
            ...
            <section className="grid md:grid-cols-2 gap-16 items-start py-10 border-b border-slate-100 pb-20 text-slate-900 w-full">
              <div className="space-y-8 text-left">
                ...
                <div className="space-y-6 text-lg text-slate-600 leading-relaxed font-serif text-justify">
          ...
          <div className="max-w-3xl w-full animate-in slide-in-from-bottom-4 text-slate-900 text-left">
            <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-xl border border-slate-50">
              ...
              <div className="min-h-[300px]">
                {currentStep === 1 && (
                  <div className="space-y-4 animate-in fade-in"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest font-sans">Occurrence Date</label><input name="date" type="date" value={formData.date} onChange={(e) => setFormData({...formData, date: e.target.value})} className="w-full bg-slate-50 p-6 rounded-2xl border-none text-xl font-serif outline-none" /></div>
                )}
                {currentStep === 2 && (
                  <div className="space-y-4 animate-in fade-in"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest font-sans">Location</label><input name="location" type="text" value={formData.location} onChange={(e) => setFormData({...formData, location: e.target.value})} placeholder="例：〇〇県 〇〇市役所 福祉課" className="w-full bg-slate-50 p-6 rounded-2xl border-none text-xl font-serif outline-none" /></div>
                )}
                {currentStep === 3 && (
                  <div className="space-y-4 animate-in fade-in"><label className="text-[10px] font-black text-slate-400 uppercase tracking-widest font-sans">Description</label><textarea name="description" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} className="w-full bg-slate-50 p-6 rounded-3xl min-h-[300px] border-none text-lg font-serif leading-relaxed outline-none" placeholder="事実を具体的に記述してください..." /></div>
                )}
                {currentStep === 4 && (
              ...
              <div className="mt-16 pt-8 border-t flex justify-between items-center w-full">
                ...
                <div className="flex gap-4">
                  {currentStep < 4 ? (
                    <button onClick={() => setCurrentStep(currentStep + 1)} className="bg-[#000666] text-white px-12 py-4 rounded-2xl font-bold uppercase text-[11px] tracking-widest shadow-lg font-sans">Next Step</button>
                  ) : (
                    <button onClick={handleFinalSubmit} disabled={isProcessing} className={`px-16 py-5 rounded-2xl font-bold text-white transition-all flex items-center justify-center gap-3 text-xs uppercase tracking-[0.2em] shadow-2xl font-sans ${isProcessing ? 'bg-slate-400' : 'bg-blue-600 hover:scale-105 active:scale-95'}`}>
