...
import { 
  ...
  Clock, Scale, Quote, BookOpen, Database, Lock
} from 'lucide-react';
...
export default function App() {
  ...
  const runDirectAIAnalysis = async () => {
    ...
    const API_KEY = "ここに新しい鍵を貼り付ける"; 

    try {
      const genAI = new GoogleGenerativeAI(API_KEY);
      const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });
      const prompt = `あなたは日本の行政法と憲法に精通した「判事」兼「調査官」です。
      以下の被害報告について、憲法25条や行政手続法7条等に照らした法的分析レポートを日本語で作成してください。
      回答は法的整合性チェックの結果を含め、400文字程度で。
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f9f9fb] p-6 text-center text-slate-900">
        <div className="max-w-md w-full bg-white p-12 rounded-[2.5rem] shadow-2xl border border-slate-100">
          ...
          <p className="text-slate-500 mb-10 leading-relaxed font-serif">法的解析を経て、あなたの証言は「Evidence」としてアーカイブされました。</p>
    ...
    <div className="min-h-screen bg-[#F8F9FA] text-[#1A202C] font-sans flex text-left">
      ...
      <aside className="hidden lg:flex flex-col fixed h-screen w-64 bg-[#f1f4f6] border-r border-[#abb3b7]/20 p-8 z-50">
        ...
        <nav className="flex-1 space-y-1">
          <button onClick={() => setView('landing')} className={`w-full flex items-center gap-3 px-4 py-3 text-[11px] font-black uppercase rounded-lg transition-all ${view==='landing' ? 'bg-white shadow-sm text-black' : 'text-slate-400'}`}><Landmark className="w-4 h-4"/> トップ</button>
        ...
        <div className="mt-auto pt-6 border-t border-slate-200">
          <a href="https://misconduct-db.vercel.app/" className="flex items-center gap-3 opacity-60 hover:opacity-100 transition-opacity mb-4">
            ...
            <p className="text-[10px] font-bold text-slate-500 uppercase tracking-tighter">不祥事ニュースへ</p>
          </a>
          <SmallDonation />
        </div>
      </aside>
        ...
        {view === 'landing' ? (
          ...
          <div className="animate-in fade-in duration-700 w-full max-w-5xl space-y-24">
            ...
            <div className="border-y-2 border-black py-8 flex justify-between items-center text-slate-900">
              ...
              <div className="hidden md:block text-right font-serif opacity-40 leading-tight">
                <p className="text-xl font-bold uppercase tracking-widest">Public Service</p>
                <p className="text-xl font-bold uppercase tracking-widest">Accountability</p>
              </div>
            </div>
            ...
            <section className="bg-[#000666] text-white p-12 md:p-20 rounded-[3rem] shadow-2xl relative overflow-hidden text-left">
              ...
              <p className="text-xl text-blue-100/80 mb-12 max-w-2xl leading-relaxed font-serif italic border-l-2 border-blue-400/30 pl-6">
                「窓口での追い返しを、憲法と法律に基づいたデータへと構造化。AIによる2段階の法的整合性チェックを搭載した行政監視プラットフォーム。」
              </p>
            ...
            {/* 3. プロジェクトの意義（READMEの内容） */}
            <div className="grid md:grid-cols-3 gap-12 text-slate-900">
              <div className="space-y-4">
                <div className="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center text-blue-600"><Scale className="w-6 h-6"/></div>
                <h4 className="text-xl font-black font-serif italic">リーガル・チェック</h4>
                <p className="text-sm text-slate-500 leading-relaxed">全ての報告は、AIによって日本国憲法第25条（生存権）および行政手続法第7条に照らして自動分析されます。</p>
              </div>
              <div className="space-y-4">
                <div className="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center text-blue-600"><Database className="w-6 h-6"/></div>
                <h4 className="text-xl font-black font-serif italic">証拠のアーカイブ</h4>
                <p className="text-sm text-slate-500 leading-relaxed">散逸しがちな「窓口での不利益な対応」を、検証可能なデータセットとして蓄積し、社会的な可視化を目指します。</p>
              </div>
              <div className="space-y-4">
                <div className="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center text-blue-600"><ShieldCheck className="w-6 h-6"/></div>
                <h4 className="text-xl font-black font-serif italic">透明性の追求</h4>
                <p className="text-sm text-slate-500 leading-relaxed">個人の怒りを「公的なエビデンス」へ。この活動は、行政の自浄作用を促すためのデジタルな草の根運動です。</p>
              </div>
            </div>

            {/* 4. 判定基準とする法的根拠（ここが欲しかった記述） */}
            <section className="py-20 border-t border-slate-100 text-slate-900">
              <div className="max-w-4xl space-y-12">
                <div className="space-y-4">
                  <h3 className="text-3xl font-black font-serif italic">Legal Framework</h3>
                  <p className="text-slate-500 font-serif leading-relaxed">私たちは、以下の法規・規範を判定の絶対的な基準としています。</p>
                </div>
                <div className="grid md:grid-cols-2 gap-16">
                  <div className="space-y-6">
                    <h5 className="font-bold flex items-center gap-2 border-b-2 border-blue-600 pb-2 w-fit">憲法 第25条</h5>
                    <p className="text-sm text-slate-600 leading-relaxed text-justify">「全て国民は、健康で文化的な最低限度の生活を営む権利を有する。」この生存権を実質的に侵害する窓口での対応は、憲法の理念に背くものです。</p>
                  </div>
                  <div className="space-y-6">
                    <h5 className="font-bold flex items-center gap-2 border-b-2 border-blue-600 pb-2 w-fit">行政手続法 第7条</h5>
                    <p className="text-sm text-slate-600 leading-relaxed text-justify">申請が届いた際、行政は遅滞なく審査を開始しなければならず、形式的な不備がない限り、受け取りを拒否することは許されません。</p>
                  </div>
                </div>
              </div>
            </section>
            ...
            {/* 5. ビジョン（引用セクション） */}
            <div className="bg-slate-100 p-16 rounded-[3rem] text-center space-y-8">
              ...
              <p className="max-w-2xl mx-auto text-slate-500 font-serif leading-relaxed italic">このデータベースは、ジャーナリスト、法曹界、そして何より現状を打破したいと願う市民のために解放されています。</p>
            </div>

            {/* 6. フッター */}
            <footer className="pt-12 pb-24 border-t border-slate-200 text-center opacity-30">
          ...
          <div className="max-w-3xl w-full animate-in slide-in-from-bottom-4 text-slate-900">
            <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-xl border border-slate-50">
              ...
              <div className="min-h-[300px]">
                ...
                {currentStep === 3 && (
                  <div className="space-y-4 animate-in fade-in">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Full Description</label>
                ...
                {currentStep === 4 && (
                  <div className="space-y-10 animate-in fade-in">
                    <div className="p-8 bg-blue-50/50 rounded-3xl border border-blue-100">
                       ...
                       <p className="text-xs text-blue-700 leading-relaxed">送信ボタンを押すと、AIがこの報告を即座に解析し、データベースへ証拠として保存します。</p>
                    </div>
                    <label className="flex items-start gap-4 cursor-pointer group">
                      ...
                      <span className="text-sm font-bold text-slate-600 group-hover:text-black transition-colors leading-relaxed">提供した情報は事実に基づき正確であることを誓約し、登録を承認します。</span>
                    </label>
                  </div>
                )}
              </div>
              <div className="mt-16 pt-8 border-t flex justify-between items-center">
                <button onClick={() => currentStep > 1 ? setCurrentStep(currentStep - 1) : setView('landing')} className="text-[11px] font-black uppercase text-slate-300 tracking-widest">Back</button>
                {currentStep < 4 ? (
                  <button onClick={() => setCurrentStep(currentStep + 1)} className="bg-[#000666] text-white px-12 py-4 rounded-2xl font-bold uppercase text-[11px] tracking-widest">Next</button>
                ) : (
                  <button onClick={handleFinalSubmit} disabled={isProcessing} className={`px-16 py-5 rounded-2xl font-bold text-white transition-all flex items-center gap-3 text-xs uppercase tracking-[0.2em] ${isProcessing ? 'bg-slate-400' : 'bg-blue-600 hover:scale-105'}`}>
