    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f9f9fb] p-6 text-center text-slate-900">
        <div className="max-w-md w-full bg-white p-12 rounded-[2.5rem] shadow-2xl border border-slate-100">
          ...
          <h2 className="text-3xl font-black mb-4 font-serif text-left leading-tight text-[#000666]">記録が<br/>完了しました</h2>
        ...
        {view === 'landing' ? (
          <div className="animate-in fade-in duration-700 w-full max-w-5xl space-y-24">
            ...
            <section className="bg-[#000666] text-white p-12 md:p-20 rounded-[3rem] shadow-2xl relative overflow-hidden text-left w-full">
              <div className="relative z-10 space-y-8">
                ...
                <div className="flex flex-wrap gap-4 pt-4">
                  <button onClick={() => setView('reporting')} className="bg-white text-[#000666] px-10 py-5 rounded-2xl font-bold text-lg shadow-xl hover:scale-105 transition-all flex items-center gap-2 font-sans">報告を作成する <ArrowRight className="w-5 h-5"/></button>
                  <a href={NOTEBOOK_URL} target="_blank" rel="noopener noreferrer" className="bg-blue-500 text-white px-10 py-5 rounded-2xl font-bold text-lg shadow-xl hover:scale-105 transition-all flex items-center gap-2 font-sans">法律・通知を検索 <Search className="w-5 h-5"/></a>
                </div>
              </div>
            </section>
            ...
            <div className="bg-slate-100 p-16 md:p-24 rounded-[3rem] text-center space-y-8 w-full">
              ...
              <h3 className="text-3xl md:text-5xl font-black font-serif tracking-tight leading-tight text-slate-900">「沈黙を記録に変え、<br/>不作為を正す力に。」</h3>
                ...
                <div className="flex gap-4">
                  {currentStep < 4 ? (
                    ...
                    <button onClick={handleFinalSubmit} disabled={isProcessing} className={`px-16 py-5 rounded-2xl font-bold text-white transition-all flex items-center justify-center gap-3 text-xs uppercase tracking-[0.2em] shadow-2xl font-sans ${isProcessing ? 'bg-slate-400' : 'bg-blue-600 hover:scale-105 active:scale-95'}`}>
                      {isProcessing ? <><Loader2 className="w-4 h-4 animate-spin" /> Analyzing & Saving...</> : "Submit to Archive"}
                    </button>
                  )}
                </div>
              </div>
