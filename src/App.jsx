...
export default function App() {
  ...
  const runDirectAIAnalysis = async () => {
    ...
    try {
      ...
      const prompt = `あなたは日本の行政法と憲法に精通した判事兼調査官です。以下の被害報告について、憲法25条や行政手続法7条等に照らした法的分析レポートを日本語で作成してください。回答は法的整合性チェックの結果を含め、400文字程度で。報告内容：${formData.description}`;
          ...
          <div className="animate-in fade-in duration-700 w-full max-w-5xl space-y-24">
            ...
            <div className="border-y-2 border-black py-8 flex justify-between items-start text-slate-900">
              <div className="flex flex-col text-left">
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-blue-600 mb-2">Visible Administrative Inaction</span>
            ...
            <section className="grid md:grid-cols-2 gap-16 items-start py-10 border-b border-slate-100 pb-20 text-slate-900">
              <div className="space-y-8 text-left">
                ...
                <div className="space-y-6 text-lg text-slate-600 leading-relaxed font-serif text-justify">
                  ...
                  <p>それはもはや個人の問題ではなく、行政の構造的な欠陥、すなわち「不作為」の決定的な証明になります。あなたの経験をデータ化し、社会へ突きつける。それがこのサイトの目的です。</p>
                </div>
              </div>
              <div className="bg-slate-50 p-10 rounded-[3rem] space-y-8 text-left">
                <div className="space-y-4">
                  <h4 className="flex items-center gap-2 font-black uppercase text-xs tracking-[0.2em] text-blue-600"><Users className="w-4 h-4"/> あなた一人ではない</h4>
                ...
                <div className="space-y-4">
                  <h4 className="flex items-center gap-2 font-black uppercase text-xs tracking-[0.2em] text-blue-600"><Database className="w-4 h-4"/> データを活かす</h4>
                  <p className="text-sm text-slate-500 leading-relaxed text-justify">蓄積されたデータは匿名化され、検証可能な形式でジャーナリストや法曹界、政策立案者に提供されます。一人の「嫌な経験」が、制度改善のための客観的な根拠になります。</p>
                </div>
              </div>
            </section>
          ...
          <div className="max-w-3xl w-full animate-in slide-in-from-bottom-4 text-slate-900 text-left">
            <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-xl border border-slate-50">
              ...
              <div className="min-h-[300px]">
                {currentStep === 1 && (
                  <div className="space-y-4 animate-in fade-in"><label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Occurrence Date</label><input name="date" type="date" value={formData.date} onChange={(e) => setFormData({...formData, date: e.target.value})} className="w-full bg-slate-50 p-6 rounded-2xl border-none text-xl font-serif outline-none" /></div>
                )}
                {currentStep === 2 && (
                  <div className="space-y-4 animate-in fade-in"><label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em]">Location</label><inp
