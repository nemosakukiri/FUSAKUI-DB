...
import { 
  ...
  Clock, Scale, Quote, BookOpen, Database, Lock, Search, ExternalLink,
  Users
} from 'lucide-react';
...
export default function App() {
  ...
  const runDirectAIAnalysis = async () => {
    ...
    const API_KEY = "import React, { useState, useEffect } from 'react';
import { 
  Gavel, Sparkles, Calendar, MapPin, FileText, ArrowRight, Newspaper, 
  ChevronLeft, CheckCircle2, ShieldCheck, ShieldAlert, Loader2, Landmark, 
  Clock, Scale, Quote, BookOpen, Database, Lock, Search, ExternalLink,
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#f9f9fb] p-6 text-center text-slate-900">
        <div className="max-w-md w-full bg-white p-12 rounded-[2.5rem] shadow-2xl border border-slate-100">
          ...
          <h2 className="text-3xl font-black mb-4 font-serif text-left leading-tight">記録が<br/>完了しました</h2>
        ...
        {view === 'landing' ? (
          <div className="animate-in fade-in duration-700 w-full max-w-5xl space-y-24">
            ...
            <div className="border-y-2 border-black py-8 flex justify-between items-start text-slate-900">
              <div className="flex flex-col text-left">
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-red-600 mb-2 font-sans">Visible Administrative Inaction</span>
              ...
              <div className="hidden md:block max-w-[250px] text-right space-y-4">
                ...
                <div className="flex justify-end gap-2 text-slate-300"><Users className="w-4 h-4"/><p className="text-[10px] font-black uppercase tracking-widest font-sans">Connective Voice</p></div>
              </div>
            </div>
            ...
            <section className="bg-[#000666] text-white p-12 md:p-20 rounded-[3rem] shadow-2xl relative overflow-hidden text-left">
              <div className="relative z-10 space-y-8">
                ...
                <div className="flex flex-wrap gap-4 pt-4">
                  ...
                  <a href={NOTEBOOK_URL} target="_blank" rel="noopener noreferrer" className="bg-blue-500 text-white px-10 py-5 rounded-2xl font-bold text-lg shadow-xl hover:scale-105 transition-all flex items-center gap-2">法律・通知を検索 <Search className="w-5 h-5"/></a>
                </div>
              </div>
            </section>

            <section className="grid md:grid-cols-2 gap-16 items-start py-10 border-b border-slate-100 pb-20 text-slate-900">
              ...
              <div className="bg-slate-50 p-10 rounded-[3rem] space-y-8 text-left">
                <div className="space-y-4">
                  <h4 className="flex items-center gap-2 font-black uppercase text-xs tracking-[0.2em] text-blue-600 font-sans"><Users className="w-4 h-4"/> あなた一人ではない</h4>
                ...
                <div className="space-y-4">
                  <h4 className="flex items-center gap-2 font-black uppercase text-xs tracking-[0.2em] text-blue-600 font-sans"><Database className="w-4 h-4"/> データを活かす</h4>
                  <p className="text-sm text-slate-500 leading-relaxed text-justify">蓄積されたデータは匿名化され、検証可能な形式でジャーナリストや法曹界へ提供されます。一人の「嫌な経験」が、制度改善のための客観的な根拠になります。</p>
                </div>
              </div>
            </section>
            ...
            <footer className="pt-12 pb-24 border-t border-slate-200 text-center opacity-30 text-slate-900">
              <p className="text-[10px] font-black uppercase tracking-[0.3em] font-sans">© 2024 The Forensic Editorial Project. Japan.</p>
            </footer>
          </div>
        ) : (
          <div className="max-w-3xl w-full animate-in slide-in-from-bottom-4 text-slate-900 text-left">
            <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-xl border border-slate-50">
              <div className="flex justify-between items-center mb-12 border-b pb-6 text-left">
                <h2 className="text-xl font-black font-serif italic text-left">Case Reporting</h2>
                <span className="text-[10px] font-black text-slate-300 uppercase tracking-widest text-right font-sans">Step {currentStep} / 4</span>
              </div>
              <div className="min-h-[300px] text-left">
                {currentStep === 1 && (
                  <div className="space-y-4 animate-in fade-in"><label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] font-sans">Occurrence Date</label><input name="date" type="date" value={formData.date} onChange={(e) => setFormData({...formData, date: e.target.value})} className="w-full bg-slate-50 p-6 rounded-2xl border-none text-xl font-serif outline-none" /></div>
                )}
                {currentStep === 2 && (
                  <div className="space-y-4 animate-in fade-in"><label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] font-sans">Location</label><input name="location" type="text" value={formData.location} onChange={(e) => setFormData({...formData, location: e.target.value})} placeholder="例：〇〇県 〇〇市役所 福祉課" className="w-full bg-slate-50 p-6 rounded-2xl border-none text-xl font-serif outline-none" /></div>
                )}
                {currentStep === 3 && (
                  <div className="space-y-4 animate-in fade-in"><label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] font-sans">Description</label><textarea name="description" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} className="w-full bg-slate-50 p-6 rounded-3xl min-h-[300px] border-none text-lg font-serif leading-relaxed outline-none" placeholder="事実を具体的に記述してください..." /></div>
                )}
                {currentStep === 4 && (
                  <div className="space-y-10 animate-in fade-in">
                    <div className="p-8 bg-blue-50/50 rounded-3xl border border-blue-100 shadow-sm text-left">
                       <h4 className="font-bold text-blue-900 mb-2 flex items-center gap-2 text-left"><ShieldCheck className="w-5 h-5"/> AI Forensic Analysis & Archiving</h4>
              ...
              <div className="mt-16 pt-8 border-t flex justify-between items-center">
                <button onClick={() => currentStep > 1 ? setCurrentStep(currentStep - 1) : setView('landing')} className="text-[11px] font-black uppercase text-slate-300 tracking-widest hover:text-slate-600 transition-all font-sans">Back</button>
                <div className="w-full md:w-auto text-right">
                  {currentStep < 4 ? (
                    <button onClick={() => setCurrentStep(currentStep + 1)} className="w-full md:w-auto bg-[#000666] text-white px-12 py-4 rounded-2xl font-bold uppercase text-[11px] tracking-widest shadow-lg font-sans">Next Step</button>
                  ) : (
                    <button onClick={handleFinalSubmit} disabled={isProcessing} className={`w-full md:w-auto px-16 py-5 rounded-2xl font-bold text-white transition-all flex items-center justify-center gap-3 text-xs uppercase tracking-[0.2em] shadow-2xl font-sans ${isProcessing ? 'bg-slate-400' : 'bg-blue-600 hover:scale-105 active:scale-95'}`}>
                      {isProcessing ? <><Loader2 className="w-4 h-4 animate-spin" /> Analyzing & Saving...</> : "Submit & Support Change"}
                    </button>
                  )}
                </div>
              </div>
